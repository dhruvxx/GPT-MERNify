...
export const userSignup = async (
    ...
    try{
        ...
        return res.status(201).json({message: "Works", name:users.name, email:users.email});
    }
    catch (error) {
        console.log(error);
        return res.status(200).json({message: "Error", cause: error.message});
...
export const userLogin = async (
    ...
    try{
        ...
        expires.setDate(expires.getDate() + 7);
        res.cookie(COOKIE_NAME, token, {path: "/", domain: "localhost", expires, httpOnly: true, signed: true});
        return res.status(201).json({message: "Works", name:user.name, email:user.email});
    }
    catch (error) {
        console.log(error);
        return res.status(200).json({message: "Error", cause: error.message});
...
export const verifyUser = async (
    ...
    try{
        ...
        return res.status(200).json({message: "Works", name:user.name, email:user.email});
    }
    catch (error) {
        console.log(error);
        return res.status(200).json({message: "Error", cause: error.message});
