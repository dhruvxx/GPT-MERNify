  ...
  useLayoutEffect(() => {
    if(auth?.isLoggedIn && auth.user){
      ...
      getUserChats().then((data) => {
        ...
        toast.success("Loaded chat", {id: "loadchats"});
      }).catch((error) => {
        console.log(error);
        toast.error("Could not load chat", {id: "loadchats"});
      });
    }
  }, [auth]);
  useEffect(() => {
  ...
  return (<Box sx={{display: "flex", flex:1, width:"100%", height:"100%", mt:3, gap:3}}>
    <Box sx={{display:{md:"flex", xs:"none", sm:"none"}, flex:0.2, flexDirection: "column"}}>
      <Box sx={{display: "flex", width: "100%", height: "60vh", bgcolor: "rgb(17,29,39)", borderRadius: 5, flexDirection: "column", mx: 3}}>
        ...
        <Typography sx={{mx: "auto", fontFamily: "work sans"}}>Hi, I am your ChatBOT friend. How can I help you today?</Typography>
        <Typography sx={{mx: "auto", fontFamily: "work sans", my: 4, p: 3}}>"Avoid sharing personal details :)"</Typography>
    ...
    <Box sx={{display: "flex", flex:{md: 0.8, xs:1, sm:1}, flexDirection: "column", px: 3, fontWeight: "600"}}>
      <Typography sx={{fontSize: "40px", color: "white", mb:2, mx: "auto"}}>GPT 3.5-Turbo</Typography>
      <Box
          sx={{
            width: "100%",
            height: "60vh",
